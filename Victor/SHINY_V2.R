...
server <- function(input, output, session) {
  data <- reactive({
    req(input$file)
    # Utiliser l'option header lors de la lecture du fichier CSV
  ...
  # Afficher les sorties print et summary
  output$model_output <- renderPrint({
    cat("\nSortie de la fonction print :\n")
    nb_model$print()
    cat("\nRésumé du modèle :\n")
    nb_model$summary()
  })
  ...
  tabPanel("Données brutes", 
           fluidRow(
             column(width = 3,
                    ...
                    checkboxInput("header", "Le fichier CSV a un en-tête", value = TRUE),  # Checkbox for header
             ...
             column(width = 9,
                    ...
                    h4("Accuracy"),
                    p("L'accuracy du modèle est de 95%"),
                    h4("Rappel"),
                    p("Le rappel du modèle est de 90%"),
...
new_data <- test_data[, -5]


model=NaiveBayes$new()
model$fit(X,y,preproc = TRUE)
