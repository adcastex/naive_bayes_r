  ...
  tabPanel("Entraîner le modèle",
           fluidRow(
             ...
             column(width = 9,
                    ...
                    h4("Accuracy"),
                    p("L'accuracy du modèle est de n%"),
                    h4("Rappel"),
...
server <- function(input, output, session) {
  ...
  observe({
    if (input$predict_new_data > 0) {
      ...
      X <- train_data[, setdiff(names(data()), input$target_variable)]
      y <- train_data[, input$target_variable]
  ...
  new_data <- reactive({
    req(input$new_file)
    read.csv(input$new_file$datapath, sep = input$separator, dec = input$decimal, header = input$header)
  })
  
  #ça doit servir a qqchose mais quoi ...
  observe({
